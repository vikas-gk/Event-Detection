...
def visualize_predictions(video_path, predictions, save_path, only_graph, vid_frames=0):

    if only_graph == 2:
        print("Prediction: ",max(predictions[0:vid_frames])*100000)
        x = range(0, vid_frames)
        y = predictions[0:vid_frames]
        plt.plot(x, y, '-')
        plt.savefig("output_vid.png")
        return 0
